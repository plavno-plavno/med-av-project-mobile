...
const useWebRtc = (isPreview?: boolean) => {
    ...
    useEffect(() => {
        if (socket && roomId) {
            ...
            socket.on('candidate', async ({ candidate }) => await handleCandidate(candidate));
            socket.on('user-joined', async ({ userId }: { userId: number }) => await handleUserJoined(userId));
    ...
    const handleUserJoined = async (userId: number) => {
        try {
            ...
            const userExists = participants.find((participant) => participant?.id === userId);
    ...
    const handleTransceiver = async ({ userId }: { mid: string | null, userId: number }) => {
