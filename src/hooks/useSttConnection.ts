...
const useSttConnection = ({ sttUrl, isAudioOn }: IUseSttConnectionProps) => {
  ...
  const [, setSttLanguage] = useState<string | undefined>(
    user?.outputLanguage?.code || ""
  )
  const sttLanguageRef = useRef<string | undefined>(
    user?.outputLanguage?.code?.toLowerCase?.()
  )
  ...
  useEffect(() => {
    sttLanguageRef.current = user?.outputLanguage?.code?.toLowerCase?.()
  }, [user])
